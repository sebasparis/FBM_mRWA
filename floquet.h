...
const cx_mat SP = {{C0,C1},{C0,C0}};
const cx_mat SM = {{C0,C0},{C1,C0}};
#endif
...
class floquet{
    ...
    cx_mat V; //driving operator
    double (*f)(double);  //driving function (TAU-periodic)

    //Hamiltonian parameters
    double *x;   //vector of H0 + A + w parameters
    double A, w;
    ...
    floquet(cx_mat(*H0_funcI)(double*), cx_mat VI, double(*fI)(double), int dimI, int nparI):
        H0_func(H0_funcI),
        V(VI),
        f(fI),
        dim(dimI),
...
void floquet::calcH0(){   //calculates and diagonalizes H0
    ...
    eig_sym(energy,eigst,symmatu(H0));//symmatu is to avoid non-hermiticity caused by floating point errors.
}

void floquet::calcFloquet(){
    ...
    //first diagonalize V
    vec V_eigval(dim,fill::none);
    cx_mat V_eigst(dim,dim,fill::none);
    eig_sym(V_eigval,V_eigst,V);

    //We work in the basis where V is diagonal
    for(it=0; it<NK; it++){
        ...
        for(i=0;i<dim;i++){
            uv1_(i,i)=exp(-CI*V_eigval[i]*(v1*dt*s/2));
            uv2_(i,i)=exp(-CI*V_eigval[i]*(v1*dt*s/2+v2*dt*s/2));
            uv3_(i,i)=exp(-CI*V_eigval[i]*(v2*dt*s/2+v3*dt*z/2));
            uv4_(i,i)=exp(-CI*V_eigval[i]*(v3*dt*z/2+v4*dt*s/2));
            uv5_(i,i)=exp(-CI*V_eigval[i]*(v4*dt*s/2+v5*dt*s/2));
            uv6_(i,i)=exp(-CI*V_eigval[i]*(v5*dt*s/2));
        }

        cx_mat uh1_(dim,dim,fill::zeros);
        cx_mat uh2_(dim,dim,fill::zeros);
        for(i=0;i<dim;i++){
            //these are the projectors of the H0 eigenvalues. We transform them because right now we're working in the V eigenbasis!
            cx_mat H0_proj_i = V_eigst.t()*kron(eigst.col(i).t(),eigst.col(i))*V_eigst;
            uh1_+=exp(-CI*energy[i]*dt*s)*H0_proj_i;
            uh2_+=exp(-CI*energy[i]*dt*z)*H0_proj_i;
        }

        //Trotter Suzuki 4th order
        u.slice(it+1)=uv6_*uh1_*uv5_*uh1_*uv4_*uh2_*uv3_*uh1_*uv2_*uh1_*uv1_*u.slice(it);
        //transform back to the original basis
        u.slice(it) = V_eigst*u.slice(it)*V_eigst.t();
    }
    u.slice(NK) = V_eigst*u.slice(NK)*V_eigst.t();
    ...
    for(it=1;it<=NK;it++){ //If don't put the <=, I get a small numerical error. I don't know exactly why this happens.
        t=t+dt;
        for(j=0;j<dim;j++){
            for(i=0;i<dim;i++){
                flst_t[it%NK](i,j)=C0;
...
cx_mat floquet::mixingMatrix(int par_i, double par_epsilon){ //calculates the hermitian matrix of eigenstate mixing
    ...
    for(int i=0;i<npar;i++) par_neighbor[i] = x[i];
